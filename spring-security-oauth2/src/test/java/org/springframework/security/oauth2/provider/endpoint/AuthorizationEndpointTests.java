...
public class AuthorizationEndpointTests {
	...
	public void testAuthorizationCodeWithCurlyBracesQueryParams() throws Exception {
		endpoint.setAuthorizationCodeServices(new StubAuthorizationCodeServices());
		AuthorizationRequest authorizationRequest = getAuthorizationRequest("foo", "http://anywhere.com?foo={b}&bar={}", null, null,
		                        Collections.singleton("code"));
	...
	public void testAuthorizationCodeWithEncodedCurlyBracesQueryParams() throws Exception {
		endpoint.setAuthorizationCodeServices(new StubAuthorizationCodeServices());
		AuthorizationRequest authorizationRequest = getAuthorizationRequest("foo", "http://anywhere.com?foo=%7Bb%7D&bar=%7B%7D", null, null,
		                        Collections.singleton("code"));
        model.put(AUTHORIZATION_REQUEST_ATTR_NAME, authorizationRequest);
        model.put(ORIGINAL_AUTHORIZATION_REQUEST_ATTR_NAME, endpoint.unmodifiableMap(authorizationRequest));
		View result = endpoint.approveOrDeny(Collections.singletonMap(OAuth2Utils.USER_OAUTH_APPROVAL, "true"), model,
